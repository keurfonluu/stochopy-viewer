...
from stochopy import BenchmarkFunction, Evolutionary, MonteCarlo
...
class GUI:
    ...
    def frame1(self):
        self._frame1 = ttk.LabelFrame(
            self.master, text="Parameters", borderwidth=2, relief="groove"
        )
        self._frame1.place(
            bordermode="outside",
            relwidth=0.99,
        ...
        function_option_menu = ttk.OptionMenu(
            self._frame1, self.function, self.function.get(), *sorted(self.FUNCOPT)
        )

        # max_iter
        max_iter_label = ttk.Label(self._frame1, text="Maximum number of iterations")
        ...
        # seed
        seed_button = ttk.Checkbutton(
            self._frame1, text="Fix seed", variable=self.fix_seed, takefocus=False
        )
        ...
        # constrain
        constrain_button = ttk.Checkbutton(
            self._frame1, text="Constrain", variable=self.constrain, takefocus=False
        )
    ...
    def frame1_pop(self):
        ...
        self._frame1.pop.place(width=170, height=25, relx=0.35, y=30, anchor="nw")
    ...
    def frame1_sync(self):
        ...
        self._frame1.sync.place(width=170, height=25, relx=0.35, y=55, anchor="nw")
    ...
    def init_widget(self):
        ...
        self._frame1.sliders.place(relwidth=0.45, relheight=1.0, relx=0.55, anchor="nw")
    ...
    def _scale(
        self, from_, to, resolution, variable, position, command=None, kwargs={}
    ):
        if command is None:
    ...
    def run(self):
        if self.check_variables():
            ...
            if solver_name in ["hastings", "hamiltonian"]:
                ...
                self.solver.sample(
                    sampler=solver_name, stepsize=stepsize, n_leap=self.n_leap.get()
                )
    ...
    def check_variables(self):
        ...
        solver_name = self.solver_name.get().lower()
        if (
            solver_name.upper() in self.EAOPT
...
def main():
    ...
    GUI(root)
    s = ttk.Style()
    s.theme_use(ttk_style[platform])
    root.mainloop()
